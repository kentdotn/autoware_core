...
The antenna is rigidly mounted on the vehicle, so this transformation is expected to be static; an antenna that moves relative to `base_link` is out of scope. Publishing it on `/tf_static` is recommended, but a setup that publishes the same fixed relation on `/tf` is supported as well: the node looks up the latest transform rather than the transform at the fix's `header.stamp`, which `/tf` cannot always provide.

If the transformation from `base_link` to the antenna cannot be obtained, the fix is skipped: `gnss_fixed` still reports the receiver's status, but no pose is published. This is expected right after startup, before the transformation has been received. It is reported as an error on `/diagnostics` so that a `header.frame_id` that never gets a transformation is visible instead of silently yielding the antenna position as the vehicle pose.

An antenna that does move relative to `base_link`, a gimbal mount for instance, is supported by setting `antenna_transform_is_dynamic`. Every fix then waits for the transformation stamped like itself instead of using the latest one, so the output is delayed by however long that transformation takes to arrive, and a fix still waiting when a fix newer than it by more than `antenna_transform_timeout_sec` arrives is dropped. A fix counts as received once its position in the map frame can be computed, so a waiting fix publishes nothing at all, `gnss_fixed` included, and a dropped one does not enter the averaging buffer either.
...
| `latest_outcome`                          | what the latest NavSatFix that reached the pose computation produced: `NoProjectorInfo`, `LocalProjector`, `NotFixed`, `Buffering`, `NoAntennaTransform` or `Published`. A fix held for its antenna transform has not reached it yet. | `NotFixed`                                                                                  | `LocalProjector`              |
| `position_buffer_size`                    | number of positions in the averaging / median buffer.                                                                                                                                                                                 | none                                                                                        | none                          |
| `pending_fix_count`                       | number of fixes held until their antenna transform arrives; always 0 unless `antenna_transform_is_dynamic` is true.                                                                                                                   | greater than 0                                                                              | none                          |
| `is_dropping_fixes_for_missing_transform` | whether the last NavSatFix that was evaluated produced no pose for lack of its antenna transform.                                                                                                                                     | none                                                                                        | dropped                       |
...
| `antenna_transform_is_dynamic`  | `boolean` | `false`          | Whether the antenna moves relative to `base_frame`. False: the latest transformation is used for every fix. True: every fix waits for the transformation stamped like itself, which delays the output.                     |
| `antenna_transform_timeout_sec` | `double`  | `0.5`            | How long a fix may wait for the transformation from its antenna frame to `base_frame`, measured between fix header stamps. Only used when `antenna_transform_is_dynamic` is true; negative values are rejected at startup. |
