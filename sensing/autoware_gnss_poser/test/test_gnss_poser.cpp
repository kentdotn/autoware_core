...
// make_pose_covariance: diagonal layout, the fix variances or the given fallback, and the given
// rotation variances. The fallback here is not the default one, so the values can only come from
// the argument.
TEST(GnssPoserHelpers, MakePoseCovarianceFillsTheDiagonal)
{
  ...
  const std::array<double, 3> fallback = {7.0, 8.0, 9.0};

  const std::array<double, 36> covariance = make_pose_covariance(fix, {4.0, 5.0, 6.0}, fallback);
  ...
  fix.position_covariance_type = NavSatFix::COVARIANCE_TYPE_UNKNOWN;
  const std::array<double, 36> defaulted = make_pose_covariance(fix, {4.0, 5.0, 6.0}, fallback);
  EXPECT_DOUBLE_EQ(defaulted[cov_xx], 7.0);
  EXPECT_DOUBLE_EQ(defaulted[cov_yy], 8.0);
  EXPECT_DOUBLE_EQ(defaulted[cov_zz], 9.0);
}
